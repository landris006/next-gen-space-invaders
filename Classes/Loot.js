...
export default class Loot {
  ...
  update({ player, dt, drops, healthBar, expBar }) {
    ...
    this.dx = this.odx * (1 - this.scatterRange / this.maxScatterRange) ** 1;
    this.dy = this.ody * (1 - this.scatterRange / this.maxScatterRange) ** 1;
    ...
    if (this.distanceFromPlayer <= player.pullInLootRange) {
      ...
      if (this.x >= player.x) {
        this.x -=
          player.pullSpeed *
          Math.max(
            (1 - this.distanceFromPlayer / player.pullInLootRange) ** 2,
            0.2
          ) *
          Math.cos(this.angle) *
          dt;
    ...
    if (this.distanceFromPlayer < Math.min(player.width, player.height) / 2) {
